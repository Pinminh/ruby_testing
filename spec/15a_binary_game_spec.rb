...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_invalid_input = '30'
        second_invalid_input = 'b'
        valid_input = '2'
        allow(game_input).to receive(:gets).and_return(
          first_invalid_input, second_invalid_input, valid_input
        )
      end
  ...
  describe '#verify_input' do
    ...
    let(:min) { 1 }
    let(:max) { 10 }
    subject(:game_verify) { BinaryGame.new(min, max) }

    context 'when given a valid input as argument' do
      input = 5

      it 'returns valid input' do
        actual = game_verify.verify_input(min, max, input)
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_large) { BinaryGame.new(100, 600) }

      it 'returns 9' do
        max = game_large.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_binary_search' do
    ...
    context 'when game_over? is false five times' do
      ...
      it 'calls display_turn_order five times' do
        expect(game_display).to receive(:display_turn_order).exactly(5).times
        game_display.display_binary_search(search_display)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:min) { 10 }
    let(:max) { 89 }
    subject(:game_turn) { described_class.new(min, max) }
    ...
    before do
      ...
      allow(game_turn).to receive(:display_guess).with(search_turn)
    ...
    it 'increases guess_count by one' do
      initial_count = game_turn.instance_variable_get(:@guess_count)
      game_turn.display_turn_order(search_turn)
      new_count = game_turn.instance_variable_get(:@guess_count)
