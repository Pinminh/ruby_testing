...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continue) { described_class.new(2, '3') }
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_four) { described_class.new(1, '1', 5) }

      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        sarcastic_phrase = "That was hard. It took you #{game_four.count} guesses!\n"
        expect { game_four.final_message }.to output(sarcastic_phrase).to_stdout
      end
    end
  end
end
