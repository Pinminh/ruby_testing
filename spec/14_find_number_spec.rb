...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      guess = 5
      let(:number_guessing) { double('random_number', value: guess) }
      subject(:game_end) { described_class.new(0, 9, number_guessing, guess) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect { low_guess_game.update_range }
          .to change { low_guess_game.min }.from(0).to(5)
      end

      it 'does not update max' do
        expect { low_guess_game.update_range }
          .not_to(change { low_guess_game.max })
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        expect { high_guess_game.update_range }
          .not_to(change { high_guess_game.min })
      end

      it 'updates max to 8' do
        expect { high_guess_game.update_range }
          .to change { high_guess_game.max }.from(9).to(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { edge_case_game.update_range }
          .to change { edge_case_game.min }.from(5).to(edge_case_game.max)
      end

      it 'does not update max' do
        expect { edge_case_game.update_range }
          .not_to(change { edge_case_game.max })
      end
    end
  end
end
